            ...
            <div className='profileInfo p-3 mt-10 rounded text-center border border-white shadow h-[400px] overflow-scroll'>
                ...
                {loading ?
                    ...
                    : <div>
                        ...
                        {edit ?
                            <div>
                                <input onChange={e => setUpdateName(e.target.value)} className='input input-sm mb-3' defaultValue={user?.displayName} type="text" /><br></br>
                                <input accept='image/*' onChange={e => setUpdateProfileImage(e.target.files[0])} type="file" className="file-input file-input-sm file-input-bordered my-4 max-w-xs" /><br></br>
                                <button onClick={handleUpdateProfile} className='btn btn-sm m-2'>Save</button>
                                <button onClick={() => setEdit(false)} className='btn btn-sm m-2'>Cancel</button>
                            </div>

                            : <h1 className='text-2xl'>{user?.displayName}</h1>
                        }
                        <div className='othersProfileInfo mb-10 mt-4'>
                            <h3>Email: <span className='text-blue-500'>{user?.email}</span></h3>
